...
const volunteerLogin = (props: Props) => {
  ...
  const submitLogin = async () => {
    ...
    if (response.data?.mentorLogin.errors) {
      setError(response.data?.mentorLogin.errors[0].message)
    }
  }
    ...
    <div className="bg-violet-900 flex flex-col-reverse md:flex-row px-4 py-4 items-center justify-center text-center align-middle h-screen">
      <div>
        ...
        <div>
          ...
          <button type="submit" className="text-white mt-3 bg-violet-700 hover:bg-violet-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center" onClick={submitLogin}>Login</button>
        </div>
        <hr className="my-4 mx-auto w-96 h-1 bg-gray-100 rounded border-0 md:my-10" />
      </div>
    </div>
